...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    // replace querySelectAll by getElementsByClassName
    var elements = document.getElementsByClassName("randomPizzaContainer");
    // change each pizza container to a new width by percentage
    var newsize = size=='1'?'25%':size=='2'?'33.33%':size=='3'?'50%':null;
...
function updatePositions() {
  ...
  // replace querySelecAll by getElementsByClassName
  var items = document.getElementsByClassName('mover');
  // cache document.body.scrollTop for use in loop
  var scrollTop = document.body.scrollTop;
  for (var i = 0; i < items.length; i++) {
    // move back into loop and i is increase and iterated inside loop
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //change number of loop form 200 to 40
  for (var i = 0; i < 40; i++) {
